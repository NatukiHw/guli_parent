package moe.tree.servicebase.util;
...
public class JWTUtil {
	...
	/**
	 * 签名 此签名为 NATUKILAB_FAKESHOP 的32位 大写 MD5
	 */
	private static final String SIGN_KEY = "C61FA7632E1E24FD0DA9A5AB339550A3";
	/**
	 * 默认的过期时间，7天
	 */
	private static final Integer DEFAULT_EXPIRES = 60 * 24 * 7;
	 ...
	 */
	public static String getToken(Map<String, Object> map, Integer expires) {
	...
	/**
	 * 对一个base64编码进行混淆 此处还可以进行replace混淆，考虑到效率问题，这里就不做啦~ 对于加密的思路还有位移、字符替换等~
	 *
	...
	private static String confoundPayload(String token) {
		...
		if (split.length != DEFAULT_TOKEN_SIZE) {
			throw new GuliException(20001, "用户令牌生成错误");
		}
		// 取出payload
